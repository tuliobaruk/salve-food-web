...
export const fetchPedidoData = async (status: string, page: number = 0, size: number = 5) => {
  try {
    const response = await axiosInstance.get(`/api/pedidos/loja/status/${status}?page=${page}&size=${size}`, {
      headers: {
...
export const definirEntregador = async (pedidoId: number, entregadorId: number) => {
  try {
    const response = await axiosInstance.put(`/api/pedidos/${pedidoId}/entregador/${entregadorId}`, {
      entregadorId,
    }, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("access_token")}`,
